...
// extern __global__ void free_device_memory();
// ==============
// Main Function
// ==============
...
int main(int argc, char** argv) {
    ...
    int *bins_d;
    int *parts_idx_d;
    // 在整个模拟过程中访问的parts数组都是在gpu上的
    init_simulation(parts_gpu, num_parts, size, &bins_d, &parts_idx_d);

    for (int step = 0; step < nsteps; ++step) {
        simulate_one_step(parts_gpu, num_parts, size, bins_d, parts_idx_d);
    ...
    fsave.close();
    cudaFree(parts_gpu);
    cudaFree(parts_idx_d);
    cudaFree(bins_d);
    // free_device_memory<<<1, 1>>>();
    delete[] parts;
}
